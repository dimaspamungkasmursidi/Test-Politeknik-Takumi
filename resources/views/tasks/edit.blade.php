    ...
    <form action="{{ route('tasks.update', $task) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <textarea name="description" rows="4" 
                class="border border-gray-300 p-3 w-full rounded-lg focus:ring focus:ring-blue-200">{{ old('description', $task->description) }}</textarea>
        </div>
        ...
        <div>
            ...
            <select name="status" 
                ...
                <option value="pending" {{ old('status', $task->status) == 'pending' ? 'selected' : '' }}>Belum Selesai</option>
                <option value="in_progress" {{ old('status', $task->status) == 'in_progress' ? 'selected' : '' }}>Sedang Berjalan</option>
